...
import { useAuthInterface } from 'types/authTypes';
import { useService } from '@xstate/react';
import { authService } from './authReducer';

/**
 * The main API for useAuth based off https://github.com/Swizec/useAuth
 *
...
export const useAuth: useAuthInterface = () => {
  ...
  const handleAuthentication = useCallback(
    async ({ postLoginRoute = '/' } = {}) => {
      ...
      if (typeof window !== 'undefined') {
        ...
        const loggedIn = await authProvider.handleLoginCallback(dispatch);
  ...
  const isAuthenticated = () => {
    return !!(state.context.expiresAt && state.context.expiresAt > new Date());
  };
